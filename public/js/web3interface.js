...
const contract_address = "0x6Fe6A96401a6134dAe16D11999164265610Db103"; // 따옴표 안에 주소값 복사 붙여넣기
...
const _shareRoom = async (name, location, price) => {
  ...
  // 화면을 업데이트 한다.
  try {
    const RS = await getRoomShareContract();
    await RS.methods.shareRoom(name, location, price).send({from: user, gas: 3000000});
    alert("방이 등록되었습니다.");
  }
  catch (err) {
	console.log(err);
...
const _getMyRents = async () => {
  ...
  try {
    const RS = await getRoomShareContract();
    const myRents = await RS.methods.getMyRents().call({from: user});
...
const _getAllRooms = async () => {
  ...
  try {
    const RS = await getRoomShareContract();
    const n = await RS.methods.roomId().call({from: user});
  
    const rooms = [];
    for (let i = 0; i < n; i++){
  	  const room = await RS.methods.roomId2room(i).call({from: user});
	  rooms.push(room);
    }
...
const _rentRoom = async (roomId, currentYear, checkInDate, checkOutDate, price) => {
  ...
  try {
    const RS = await getRoomShareContract();
	await RS.methods.rentRoom(roomId, currentYear, checkInDate, checkOutDate).send({from: user, gas: 3000000, value: price * Math.pow(10, 15)});
	alert("대여가 완료되었습니다.");
  }
  catch(err) {
	console.log(err);
	const balance = await getBalance(user) * Math.pow(10, 18);
	if (balance < price * Math.pow(10, 15)){
	  alert("잔액이 부족합니다.");
	}
	else{
...
const _recommendDate = async (roomId, checkInDate, checkOutDate) => {
  ...
  try {
    const RS = await getRoomShareContract();
    const dates = await RS.methods.recommendDate(roomId, checkInDate, checkOutDate).call({from: user});

    const start = dateFromDay(currentYear, dates[0]).toDateString();
    const end = dateFromDay(currentYear, dates[1]).toDateString();

    alert(`이 방은 ${start}부터 ${end}까지 대여되어 있습니다.`);
  }
  catch (err) {
	console.log(err);
  }
...
const getRoomRentHistory = async () => {
  ...
  try {
    ...
    history = await RS.methods.getRoomRentHistory(roomId).call({from: user});
...
const markRoomAsInactive = async () => {
  ...
  await _markRoomAsInactive(roomId);
  _updateRooms();
}
...
const _markRoomAsInactive = async (_roomId) => {
  ...
  try{
    const RS = await getRoomShareContract();
    await RS.methods.markRoomAsInactive(_roomId).send({from: user, gas: 3000000});
    alert("해당 방이 비활성화되었습니다.");
  }
  catch (err) {
    console.log(err);
	alert("권한이 없습니다.");
...
const intializeRoomShare = async () => {
  const roomId = roomIddom.value;
  await _intializeRoomShare(roomId);
  _updateRents();
}

const _intializeRoomShare = async (_roomId) => {
  // optional 2: 대여 초기화
  // 소유한 방 중에서 선택한 방의 대여된 일정을 모두 초기화 한다.
  try {
    const RS = await getRoomShareContract();
    await RS.methods.initializeRoomShare(_roomId).send({from: user, gas: 3000000});
    alert("해당 방의 대여된 일정이 초기화되었습니다.");
  }
  catch (err) {
	console.log(err);
	alert("권한이 없습니다.");
